...
YOUR ASSIGNMENT:
Implement components/Accordion.tsx, LogoRow.tsx, LeaderboardRow.tsx. Accordion native server-compatible details, grouped categories h3 headings and unique aria-labelledby; no rotation animations (only card hover motion allowed); default first grouped item open, explicit null disables default and index selects flattened item. Do not pass defaultOpen (unsupported HTML) use open initial attributes or safe uncontrolled wrapper. Keep flat old items API. Category headings no synthetic FAQs. LogoRow grayscale assets object-contain, no heading, each real name accessible exactly once; absent src shows labelled gray/surface-sunk block containing name. No downloads. LeaderboardRow narrow-safe grid: rank, 48px avatar/initials, handle, right-aligned expanded money. At 375px should not overflow even long handle and 6-figure money; use small value mobile, h3 at sm+ and responsive wrapping if needed; no truncating amount. Avatar provided only local actual paths, placeholder sentinel no invalid src. Use formatUsd existing cents formatter; decorative avatar alt empty because handle is adjacent.
