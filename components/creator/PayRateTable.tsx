    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-col leading-tight">
          <h2 id="pay-rate-heading" className="font-display-sm text-[22px]">
      ...
      <table className="w-full text-[15px] border-separate border-spacing-y-1">
        <tbody>
          {tiers.map((t, i) => {
            ...
            return (
              <tr key={t.minViews} className={top ? "bg-lime text-surface-dark" : "bg-surface-sunk"}>
                <th scope="row" className="py-3 pl-4 text-left font-semibold rounded-l-[8px]">
                  {CAMPAIGN_DETAIL.payRate.range(
                    formatViews(t.minViews),
                    t.maxViews === null ? null : formatViews(t.maxViews)
                  )}
                </th>
                <td className={`py-3 pr-4 text-right font-display text-[22px] md:text-[26px] rounded-r-[8px] ${top ? "text-surface-dark" : "text-ink"}`}>
